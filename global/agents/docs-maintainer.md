...
Read only the code and doc files relevant to the specific changes being documented. Never sweep the entire docs tree or read every doc file end to end; use targeted greps and read only the regions around a change. Prefer minimal, factual updates that preserve existing structure. Derive the current product direction from the repository's implementation, README, AGENTS.md, and architecture documentation rather than carrying assumptions from other projects.
   ...
   Once a PR is created or you decide no changes are needed, that is the terminal state: post the final summary and stop. Do not re-verify completed work, re-scan docs, or re-check the PR. If a continuation prompt arrives after that, reply with a one-line final status and stop.
